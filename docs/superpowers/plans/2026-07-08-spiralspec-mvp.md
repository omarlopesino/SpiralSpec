...
src/core/types.ts        domain types (Task, Spec, statuses, issues)
...
skills/{define,plan,implement,verify,release}.md   agent-neutral skill sources
templates/{context.md, acceptance-criteria.md, solution.md, status-readme.md, status-release.md}
...
### Task 2: Core types and frontmatter parsing

**Files:**
- Create: `src/core/types.ts`, `src/core/frontmatter.ts`
...
Expected: FAIL — cannot resolve `../src/core/frontmatter.js`.

- [ ] **Step 3: Implement types.ts (exact contents from Interfaces above) and frontmatter.ts**
...
```bash
git add src/core/types.ts src/core/frontmatter.ts test/frontmatter.test.ts
git commit -m "feat: core types and task/spec frontmatter parsing"
```
...
function spec(tasks: TaskData[], invalid: SpecData['invalid'] = []): SpecData {
  return { slug: 'demo', dir: '/tmp/demo', fm: { name: 'Demo', autonomy: 'medium' }, tasks, invalid };
}
...
git commit -m "feat: spec validation (refs, scopes, sections, cycles, overlap rule)"
```
...
- Create fixtures: `test/fixtures/project/.spiralspec.yml`, `test/fixtures/project/specs/demo/context.md`, `.../demo/tasks/setup-db.md`, `.../demo/tasks/field-map.md`, `.../demo/tasks/migrate-users.md`, `.../demo/tasks/broken.md`
...
describe('loadSpec', () => {
  it('loads tasks and reports invalid files', () => {
...
export function loadSpec(specsDir: string, slug: string): SpecData {
  ...
  return { slug, dir, fm, tasks, invalid };
}
...
**Files:**
- Create: `templates/context.md`, `templates/acceptance-criteria.md`, `templates/solution.md`, `templates/status-readme.md`, `templates/status-release.md`, `src/adapters/scaffold.ts`
...
**Files:**
- Create: `skills/define.md`, `skills/plan.md`, `skills/implement.md`, `skills/verify.md`, `skills/release.md`

**Interfaces:**
- Consumes: nothing (pure content).
- Produces: five agent-neutral skill sources. Each has frontmatter `id`, `title`, `description`, `args`; the body is the full instruction text. Task 13's adapters and Task 14's installer consume them via `loadSkillSources()`.
...
Turn a defined spec into tasks. Plan what is necessary to start, not
everything: undecided details are decided during implementation or postponed
explicitly. Re-running this skill with user feedback refines the existing plan
(plan review); update tasks in place, never regenerate from scratch.

## Steps

1. Run `spiralspec status <spec> --json`. Read `context.md`,
   `acceptance-criteria.md`, `solution.md`. Note the `autonomy` level.
...
4. Create one file per task in `tasks/` using the template below. Every task
   starts as `status: backlog`.
5. Run `spiralspec validate <spec>` and fix every issue until it prints OK.
6. Update `status/README.md`: rewrite `# Current state` and `# Next steps`
   (tell the user to review the tasks and flip approved ones
   `backlog → todo`).
7. Present the plan: each task's goal, ground order, and scope. Ask for
   feedback.
...
- [ ] **Step 3: Write skills/implement.md**
...
- [ ] **Step 4: Write skills/verify.md and skills/release.md**
...
```bash
git add skills/
git commit -m "feat: agent-neutral skill pack sources (define/plan/implement/verify/release)"
```
...
describe('loadSkillSources', () => {
  it('loads the five packaged skills', () => {
    const ids = loadSkillSources().map((s) => s.id).sort();
    expect(ids).toEqual(['define', 'implement', 'plan', 'release', 'verify']);
...
- **Markdown is the database.** A spec is a folder (`specs/<slug>/`) of plain
  ...
  you), `tasks/*.md` (scaffolded by the agent, approved by you),
  `status/README.md` + `status/release.md` (living reports). AI-scaffolded,
  human-edited, agent-agnostic.
...
This scaffolds `.spiralspec.yml`, a `specs/` root, and installs five
commands/skills into your agent(s): `/spiral:define`, `/spiral:plan`,
`/spiral:implement`, `/spiral:verify`, `/spiral:release` (OpenCode:
`/spiral-define`, …).
...
- Spec traceability: D1→Tasks 1,10; D2/D3→3,5,7; D4→9,11; D5→2,6; D6→11,12; D7/D8→12 (skill content); D9→14; D10→12 (implement skill).
