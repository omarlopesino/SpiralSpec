...
- The ledger is presented to the user before expansion; how hard planning stops for verification follows the autonomy dial (D13): low = wait for verification, medium = one "proceed or review?" question, high = proceed and log the ledger as unverified in status/README. Expansion is lazy either way — "expand and implement create-field-map now, leave the rest for later" is a normal flow.
